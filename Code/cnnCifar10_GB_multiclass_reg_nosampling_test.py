...
def calculate_residuals_reg(y_train,y_pred,step_size):
    error=[]
    for i in range(0,y_train.shape[0]):
        pred=y_pred[i]
        ytrain=y_train[i]
        ...
        for j in range(0,pred.shape[0]):
            residual.append(ytrain[j]-(step_size*pred[j]))
        residual=np.asarray(residual)
        error.append(residual)
    error=np.asarray(error)
    return error
...
if __name__ == '__main__':
  ...
  for steps in range(0,no_of_boosting_iter):
     ...
     error=calculate_residuals_reg(error,y_predict_reg,step_size)
  ...
  for i in range(0,len(pred_each_model)):
      ...
      prediction=np.multiply(prediction_per_learner,step_size)
      result=np.add(result,prediction)
