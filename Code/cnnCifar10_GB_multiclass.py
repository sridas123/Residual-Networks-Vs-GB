...
if __name__ == '__main__':
  ...
  print "The y_train   ::   ", y_train[0:5]
  cnn = cnn_n.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, validation_data=(x_test,y_test),shuffle=True)
  #y_predict=cnn_n.predict(x_train, batch_size=None, verbose=0, steps=None)
  y_predict=cnn_n.predict(x_train, verbose=0)
  #print y_predict
  error = calculate_residuals_cls(y_train,y_predict)
  print "The residuals from Classifier  ::",  error[0:5]
  ...
  for steps in range(1,no_of_boosting_iter):
     #x_train,y_train = splitdataset(x_train_full, error, subset_train)
     ...
     #y_predict_reg=cnn_n_reg.predict(x_train, batch_size=None, verbose=0, steps=None)
     y_predict_reg=cnn_n_reg.predict(x_train, verbose=0)
     print "The predictions from regressor", y_predict_reg[0:5]
     error=calculate_residuals_reg(error,y_predict_reg)
  ...
  for k in range(0,pred_each_model[0].shape[0]):
      ...
      vec_sum=np.true_divide(vec_sum,2)
